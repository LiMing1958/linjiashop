  ...
  <el-table :data="formatData" :row-style="showRow" v-bind="$attrs">
    <el-table-column v-if="columns.length===0" width="80">
      <template slot-scope="scope">
        <span :key="space" class="ms-tree-space" v-for="space in scope.row._level"></span>
        <span @click="toggleExpanded(scope.$index)" class="tree-ctrl" v-if="iconShow(0,scope.row)">
          <i class="el-icon-arrow-right" v-if="!scope.row._expanded"></i>
    ...
    <el-table-column :key="column.value" :label="column.text" :width="column.width" v-else v-for="(column, index) in columns">
      <template slot-scope="scope">
        <span :key="space" class="ms-tree-space" v-for="space in scope.row._level" v-if="index === 0"></span>
